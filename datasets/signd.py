...
def make_dataset(root, source):
    ...
    if not os.path.exists(source):
        print("Setting file %s for sign dataset doesn't exist." % (source))
        sys.exit()
    else:
        clips = []
...
def ReadSegmentRGB(path, offsets, new_height, new_width, new_length, is_color, name_pattern, duration, d_name_pattern=None):
    if is_color:
    ...
    for offset_id in range(len(offsets)):
        ...
        for length_id in range(1, new_length+1):
            ...
            d_frame_name = d_name_pattern % (moded_loaded_frame_index)
            d_frame_path = path + "/" + d_frame_name
            d_img_origin = cv2.imread(frame_path, cv_read_flag)
            
            if d_img_origin is None:
                print("Could not load file %s" % (d_frame_path))
                sys.exit()
                
            if new_width > 0 and new_height > 0:
                cv_img = cv2.resize(cv_img_origin, (new_width, new_height), interpolation)
                d_img = cv2.resize(d_img_origin, (new_width, new_height), interpolation)
